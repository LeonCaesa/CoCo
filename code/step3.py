...
def Callback_CDS(Xi):
    global Nfeval, l2_a3, muV_a3, sigmaV_a3, mat_values, cds_values#, t_cds, T_cds
    print(
        '{0: 4d}     k1:{1:.4f}     xi1:{2: .4f}    k2:{3:.4f}    xi2:{4:.4f}     l32:{5:.4f}    loss:{6:.4f}'.format(
            Nfeval,
            Xi[0], Xi[1], Xi[2], Xi[3], Xi[4], optimize_cds(Xi, l2_a3, muV_a3, sigmaV_a3, mat_values, 0, cds_values
                         )))
    Nfeval += 1
...
if __name__ == '__main__':
    ...
    cds_data['Price'] = cds_data['Price'].apply(lambda x: x.replace(',', '')).astype(float) / 10000
    ...
    if save_param:
        ...
        intervention_param.to_csv('../param/Intervention0131_' + file_name.split('.')[0] + '.csv', index=False)
